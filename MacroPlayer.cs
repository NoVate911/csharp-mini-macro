...
namespace MiniMacro
{
    internal sealed class MacroPlayer
    {
        ...
        {
            _desktop = VirtualDesktop.Capture();
            ...
            return Task.Run(async () =>
            {
                ...
                {
                    // MonitorJerkAsync otherwise runs forever after a normal last action.
                    _cts?.Cancel();
                    await jerkTask;
        ...
        private void SendMouseButton(string? button, bool down, double normX, double normY)
        {
            ...
            if (SendInput(2, new[] { move, click }, Marshal.SizeOf<INPUT>()) == 2)
                UpdateLastSent(normX, normY);
        }
        ...
        {
            int absX = VirtualDesktop.Absolute(normX);
            int absY = VirtualDesktop.Absolute(normY);
            return (absX, absY);
        }
        ...
        {
            _lastSentX = _desktop.PixelX(normX);
            _lastSentY = _desktop.PixelY(normY);
        }
    }
}
